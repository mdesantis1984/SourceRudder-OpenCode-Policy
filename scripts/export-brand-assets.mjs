...
function probe(command) {
  ...
  if (result.error?.code === "ENOENT" || result.status === 127) return false;
  if (result.error) throw new Error(`cannot execute ${command}: ${result.error.message}`);
  if (result.status !== 0) throw new Error(`${command} is present but failed its version check: ${result.stderr}`);
  return true;
}
const modern = probe("magick");
const transform = modern ? ["magick"] : probe("convert") ? ["convert"] : null;
const identify = modern ? ["magick", "identify"] : probe("identify") ? ["identify"] : null;
if (!transform || !identify) throw new Error("ImageMagick 7 'magick' or ImageMagick 6 'convert' plus 'identify' is required");
function run(command, ...args) {
  const result = spawnSync(command[0], [...command.slice(1), ...args], { encoding: "utf8" });
  if (result.error) throw new Error(`cannot execute ${command.join(" ")}: ${result.error.message}`);
  if (result.status !== 0) throw new Error(result.stderr || `${command.join(" ")} failed`);
}
...
function shield(target, size) {
  run(transform, source, "-gravity", "center", "-crop", "809x809+0+0", "+repage", "-resize", `${size}x${size}!`, "-strip", "-colorspace", "sRGB", "-define", "png:color-type=2", target);
}
...
async function build(dir) {
  ...
  run(transform, source, "-resize", "1280x640^", "-gravity", "center", "-extent", "1280x640", "-strip", "-colorspace", "sRGB", "-quality", "85", "-define", "png:color-type=2", resolve(dir, files[0]));
...
const sourceMetadata = spawnSync(identify[0], [...identify.slice(1), "-format", "%w %h", source], { encoding: "utf8" });
if (sourceMetadata.error) throw new Error(`cannot execute ${identify.join(" ")}: ${sourceMetadata.error.message}`);
if (sourceMetadata.status !== 0 || sourceMetadata.stdout.trim() !== `${manifest.immutable_inputs[0].width} ${manifest.immutable_inputs[0].height}`) throw new Error("approved key art metadata does not match the manifest");
